...
const Hero = () => {
	...
	return (
		<motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.75, ease: 'easeOut' }} exit={{ opacity: 0 }} className='flex flex-col items-center justify-center space-y-6 h-full '>
			...
			<div className='sm:space-x-4 space-y-4 sm:space-y-0 space-x-0 flex flex-col sm:flex-row w-full sm:w-fit sm:flex-none'>
				<Button onClick={() => openInNewTab('http://Wa.me//6281936379564')} className='px-[32px]' variant='premium'>
					Hire Me
				</Button>
				<Button onClick={() => openInNewTab('https://github.com/anangfirmansyahputra')} className='px-[32px]' variant={'secondary'}>
					Github
				</Button>
			</div>
		</motion.section>
